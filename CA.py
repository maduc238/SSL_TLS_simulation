...
class _signature:
    ...
    _algorithm = '1.2.840.10045.4.3.3'          # sha384ECDSA
...
def process_packet(source, data):
    ...
    if data[0] == 1:
        ...
        signature = sign_message(int(hashlib.sha384(encoded_TBSCertificate).hexdigest(),16), priv_ECC)
        signature = foo_3279.encode('Dss-Sig-Value', {'r': signature[0], 's': signature[1]})
