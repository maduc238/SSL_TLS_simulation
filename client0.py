...
def Action2(dest):
    user_data = [b'client_hello']
    if PartnerCer == None:
        user_data.append(b'certificate_request')
    
    data = b"\x00" + dest
    for chunk in user_data:
        data += b"\x16\x03\x00" + len(chunk).to_bytes(2, 'big') + chunk
    Client.send(data)
...
def process_packet(source, data):
    ...
    for (itr,content) in enumerate(content_type):
        ...
        if content == 21:   # alert
            # print(all_data)
            if all_data[itr] == b'no_certificate':
                print('Partner không có certificate')
            if all_data[itr] == b'bad_certificate':
                print('Partner đọc Certificate bị gián đoạn') 

        if content == 22:   # handshake
            # print(all_data)
            if all_data[itr] == b'server_hello':
                print('Nhận được hello từ server',source)
                ...
                try:
                    ...
                    if CerPem.valid:
                        ...
                        print('Gửi certificate cho User',source)
